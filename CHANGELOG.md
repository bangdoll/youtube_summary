...
- **Visual Element Fixes (內部素材修正)**:
    - **變形修正 (Smart Fit)**: 針對投影片內部的圖表、照片與圖示，不再強制填滿 AI 預測方框。改採 **Contain** 模式，保持原始長寬比縮放並置中，徹底解決圓餅圖變橢圓、照片人物變形的問題。
    - **裁切安全邊距 (Safety Crop)**: 在裁切內部素材時自動增加 **20px 安全邊距**，防止圖表邊緣線條或陰影被意外切除。

## [v7.3.1] - 2026-01-21
### 🐛 錯誤修復 (Fixes)
- **PPTX 圖片比例 (Aspect Ratio)**:
    - 修正了非 16:9 文件 (如 A4、4:3) 在生成簡報時圖片被強制拉伸變形的問題。
    - 實作 **"Smart Fit" (智慧縮放)** 邏輯：自動計算最佳比例將圖片「放入」投影片並置中 (Contain Mode)，並以背景色填充留白。
    - **座標自動校正**: 確保所有 AI 偵測到的文字與圖表 (BBox) 能跟隨縮放後的圖片準確對齊。
